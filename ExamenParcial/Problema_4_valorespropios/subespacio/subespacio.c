...
double **subespacio(double **matrix, int m, int number_eigen, double **eigen_valores_, int menores)
{
    ...
    while(condition && iteration < LIMITE)
    {
        ...
        if(menores)
            ...
            eigen_traspuesta = eigen_mayores(matrix, m, m, &number, evalues, number_eigen, ONE, eigen_traspuesta);
