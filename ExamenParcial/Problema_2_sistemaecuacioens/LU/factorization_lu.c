...
int solve_lu(double **matrix, int m, double *x, double *z)
{
    ...
    double **inversa_t, **inversa;
    FILE *file;


    file = fopen("Resultados/inversa.mtx", "w");
    ...
    L_1 = (double **)calloc(m , sizeof(double *));
    U_1 = (double **)calloc(m , sizeof(double *));

    for(i = 0; i < m; i++)
    {
        *(L_1 + i) =  (double *)calloc(m, sizeof(double));
        *(U_1 + i) = (double *)calloc(m, sizeof(double));
    }

    success = factorization_lu(matrix, m, x, z);
    ...
    for(i = 0; i < m; i++)
    {
        triangular_infer_lu(matrix, m, *(L_1 + i), *(y + i));
        triangular_super_lu(matrix, m, *(U_1 + i), *(y + i));
    }

    inversa_t = dot_matrix(L_1, U_1, m, m, m, m);
    inversa = traspuesta(inversa_t, m , m);
    ...
    printf("Se ha guardado en Resultados/inversa.mtx\n");
    free_matrix(inversa, m);
    free_matrix(inversa_t, m);
    free_expanded_matrix(L_1, m);
    free_expanded_matrix(U_1, m);
    fclose(file);
